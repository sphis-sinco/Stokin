...
class CharacterEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function update(elapsed:Float)
	{
		...
		{
			#if TOUCH_CONTROLS_ALLOWED
			if ((FlxG.keys.justPressed.W || touchPad.buttonV.justPressed) && !touchPad.buttonG.pressed && (changedAnim = true))
				curAnim--;
			else if ((FlxG.keys.justPressed.S || touchPad.buttonD.justPressed) && !touchPad.buttonG.pressed && (changedAnim = true))
				curAnim++;
			#else
